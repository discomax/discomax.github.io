const MAP_KEY =
  "pk.eyJ1IjoiZGlzY29tYXgiLCJhIjoiY2p0bGtwdjdvMGN0ZzRhcGVoaHBrdGRvNiJ9.1FRcT1k-Vdo3ZLHVSp28Fg";
// Creating map object
...
d3.json("final.json", function(data) {
  ...
  function chooseArray(price, marker) {
    switch (price) {
      case "$$$$":
        greenRestaurants.push(marker);
      case "$$$":
        violetRestaurants.push(marker);
      case "$$":
        yellowRestaurants.push(marker);
      case "$":
        redRestaurants.push(marker);
    }
  }
  ...
  for (var i = 0; i < restaurants.length; i++) {
    ...
    var restaurantMarker = L.marker(rLocation, {
      icon: chooseMarker(rPrice),
      title: rName,
      riseOnHover: true
